...
from langgraph_api.examples.open_deep_research.langgraph.configuration import Configuration
from langgraph_api.examples.open_deep_research.langgraph.prompts import report_planner_query_writer_instructions, \
    report_planner_instructions, query_writer_instructions, section_writer_instructions, section_grader_instructions, \
    final_section_writer_instructions
from langgraph_api.examples.open_deep_research.langgraph.state import ReportState, Queries, Sections, SectionState, \
    Feedback, ReportStateInput, ReportStateOutput, SectionOutputState
from langgraph_api.examples.open_deep_research.langgraph.utils import tavily_search_async, \
    deduplicate_and_format_sources, perplexity_search, format_sections, google_search

from langgraph_api.examples.open_deep_research.langgraph.state import SearchQuery, Section
...
async def generate_report_plan(state: ReportState, config: RunnableConfig):
    ...
    # Generate queries  
    try:
        results = structured_llm.invoke([SystemMessage(content=system_instructions_query)]+[HumanMessage(content="生成有助于规划报告各部分内容的搜索查询.")])
    except:
        results = Queries(queries=[SearchQuery(search_query=topic)])
    ...
    if search_api == "tavily":
        ...
        search_results = google_search(query_list)
        source_str = deduplicate_and_format_sources(search_results, max_tokens_per_source=1000, include_raw_content=False)
    else:
    ...
    try:
        report_sections = structured_llm.invoke([SystemMessage(content=system_instructions_sections)]+[HumanMessage(content="生成报告的各个部分。你的回复必须包含一个名为“sections”的字段，该字段包含各个部分的列表。每个部分必须有: name, description, plan, research, and content fields.")])
    except:
        report_sections = Sections(report_sections=[Section(description="")])
...
def human_feedback(state: ReportState, config: RunnableConfig) -> Command[Literal["generate_report_plan","build_section_with_web_research"]]:
    ...
    request: HumanInterrupt = {
        ...
        "action_request": {"action": '测试技能', "args": {}},
        "config": {
            "allow_ignore": True,
            "allow_respond": True,
            "allow_edit": True,
        ...
        "description": f"Please provide feedback on the following report plan. \n\n{sections_str}\n\n Does the report plan meet your needs? Pass 'true' to approve the report plan or provide feedback to regenerate the report plan:",
    }

    # feedback = interrupt([request])[0]
    feedback = interrupt([request])[0]

    if feedback["type"] == "ignore":
        return Command(goto="generate_report_plan",
                       update={"feedback_on_report_plan": feedback})
    elif feedback["type"] == "accept":
        return Command(goto=[
...
def generate_queries(state: SectionState, config: RunnableConfig):
    ...
    # Generate queries  
    queries = structured_llm.invoke([SystemMessage(content=system_instructions)]+[HumanMessage(content="生成主题相关的搜索查询.")])
...
async def main():
    thread_config = {"configurable": {"thread_id": "823094cd-accb-4e37-abba-3a27e6eddd4e"}}

    async for msg in graph.astream({'topic': '多智能体框架都有哪些，对比一下，用中文回答'},
    ...
    async for msg in graph.astream(Command(resume=True),
                                   config=thread_config):
        pprint(msg)
