...
_skill_client: SkillRequest

glo.set_base_host(AGENT_AUTH_ENDPOINT)


async def start_skill_client() -> None:
    global _skill_client
    _skill_client = SkillRequest()


async def stop_skill_client() -> None:
    global _skill_client
    await _skill_client.close()
    del _skill_client


def get_skill_client() -> SkillRequest:
    if "_skill_client" not in globals():
        # just for test
        return SkillRequest()
    return _skill_client
