...
from langgraph_api.config import LANGSMITH_TENANT_ID, LANGSERVE_PLUGINS
...
async def register_plugin(detail: dict, config: dict | None) -> None:
    """Register a plugin."""
    ...
    if hasattr(detail, 'data') and detail.data and 'id' in detail.data:
        tool_str = SkillGenerator.generate(detail.data)
        try:
            tool = Tool().from_code(tool_str)
            plugin_id = detail.data.get("id")
            plugin_name = detail.data.get("name")
            global PLUGINS
            PLUGINS[plugin_id] = tool

            async with connect() as conn:
                await Plugins.put(
                    conn,
                    # str(uuid5(NAMESPACE_GRAPH, plugin_id)),
                    plugin_id,
            ...
            await logger.ainfo(f"Registering plugin with id '{plugin_id}', name '{plugin_name}'")
        except Exception as e:
            logger.error(f"当前技能加载失败：{tool_str}， error: {e}")
...
async def collect_plugins_from_env(register: bool = False) -> None:
    """Return plugins from env"""
    plugins_str = LANGSERVE_PLUGINS
    config_per_graph = _load_graph_config_from_env() or {}
    if plugins_str:
        plugins = json.loads(plugins_str)

        client = get_skill_client()
        details = await client.a_skill_details(skill_ids=plugins.keys())

        if register:
            for detail in details:
                await register_plugin(detail, config_per_graph)
